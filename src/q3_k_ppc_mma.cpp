// q3_k_ppc_mma.cpp
//
// POWER10/POWER11 MMA GEMM for Q3_K x Q8_K, completing the standard
// K-quant family. Q3_K_M models use this format for most tensors.
//
// Format (ggml-common.h):
//   Q3_K: superblock of 256; per-16 6-bit scales (sc - 32, decoded from
//         the 12-byte aux packing); element value
//             d * (sc-32) * (code - (hbit ? 0 : 4))
//         = d * (sc-32) * (q' - 4),   q' = code | (hbit << 2)  in 0..7
//         chunk c (16 elems), h = c/8, idx = c%8, j = idx/2, lo = idx%2:
//           code = (qs[32h + 16lo + l] >> 2j) & 3
//           hbit = (hmask[16lo + l] >> (4h + j)) & 1
//
// Offset form with the SAME d*sc factor on both terms, exactly like
// Q6_K: the correction folds into the main FMA as a vec_msub with
// TS = dB * 4 * bsums[c]. 16-deep chunks (per-16 scales); unsigned q'
// on the unsigned GER operand. Tests: -DQ3K_TEST.
...
static_assert(sizeof(block_q3_K) == QK_K/8 + QK_K/4 + 12 + sizeof(ggml_half), "bad q3_K");
...
typedef struct {
    ...
    vfl dB[KC_SB][2];
    vfl TS[KC_CH16][2];                    // dB * 4 * bsums[c] per column
} b3k_t;
...
// 16 unsigned 3-bit codes q' = code | (hbit << 2) of chunk c (0..15).
static inline vuc q3k_codes16(const uint8_t * qs, const uint8_t * hmask, int c) {
    const int h = c >> 3, idx = c & 7, j = idx >> 1, lo = idx & 1;
    vuc code = vec_and(vec_sr(load16u((const uint8_t *)(qs) + (32*h + 16*lo)),
                              vec_splats((unsigned char)(2*j))),
                       vec_splats((unsigned char)3));
    vuc hb = vec_sl(vec_and(vec_sr(load16u((const uint8_t *)(hmask) + (16*lo)),
                                   vec_splats((unsigned char)(4*h + j))),
                            vec_splats((unsigned char)1)),
                    vec_splats((unsigned char)2));
    return vec_or(code, hb);
}
...
extern "C" size_t q3k_apack_size(int64_t m, int64_t k) {
    return (((size_t)(rt(m) * sl(k)) * sizeof(a3k_t)) + 63) & ~(size_t)63;
}
extern "C" size_t q3k_bpack_size(int64_t n, int64_t k) {
    return (((size_t)(ct(n) * sl(k)) * sizeof(b3k_t)) + 63) & ~(size_t)63;
}

extern "C" void q3k_repack_a(const block_q3_K * A, int64_t lda,
                             int64_t m, int64_t k, void * packed) {
    a3k_t * P = (a3k_t *)packed;
    ...
    for (int64_t s = 0; s < ns; s++) {
        ...
        for (int64_t b = 0; b < nsl; b++) {
            const block_q3_K * bp[MR]; float d[MR]; int8_t sc6[MR][16];
            for (int r = 0; r < MR; r++) {
                int64_t rr = it*MR + r; if (rr >= m) rr = m - 1;
                bp[r] = &A[rr*lda + sb0 + b];
                d[r]  = fp16_to_fp32(bp[r]->d);
                // decode 16 x 6-bit scales from the 12-byte packing
                const uint32_t kmask1 = 0x03030303, kmask2 = 0x0f0f0f0f;
                uint32_t aux[4];
                memcpy(aux, bp[r]->scales, 12);
                uint32_t tmp = aux[2];
            ...
            for (int c = 0; c < 16; c++) {
                ...
                for (int g = 0; g < 4; g++)
                    T->dsc[ch][g] = (vfl){
                        d[4*g+0]*(sc6[4*g+0][c]-32), d[4*g+1]*(sc6[4*g+1][c]-32),
                        d[4*g+2]*(sc6[4*g+2][c]-32), d[4*g+3]*(sc6[4*g+3][c]-32) };
                vui rows4[4];
                for (int g = 0; g < 4; g++) {
                    for (int r = 0; r < 4; r++)
                        rows4[r] = (vui)q3k_codes16(bp[4*g + r]->qs, bp[4*g + r]->hmask, c);
...
extern "C" void q3k_pack_b(const block_q8_K * B, int64_t ldb,
                           int64_t n, int64_t k, void * packed) {
    b3k_t * P = (b3k_t *)packed;
    const int64_t nsb = k/QK_K, ns = sl(k);
    ...
    for (int64_t s = 0; s < ns; s++) {
        ...
        for (int64_t b = 0; b < nsl; b++) {
            ...
            for (int c = 0; c < 16; c++) {
                ...
                float TS[NR];
                for (int j = 0; j < NR; j++)
                    TS[j] = dB[j] * 4.0f * (float)yb[j]->bsums[c];
...
static void kernel3k_16x8(const a3k_t * PA, const b3k_t * PB,
...
extern "C" void q3k_gemm_packed(int64_t m, int64_t n, int64_t k,
                                const void * packedA, const void * packedB,
                                float * C, int64_t ldc, int ith, int nth) {
    const a3k_t * PA = (const a3k_t *)packedA;
    const b3k_t * PB = (const b3k_t *)packedB;
    ...
    for (int64_t it = t0; it < t1; it++) {
        ...
        for (int64_t jt = 0; jt < njt; jt++) {
            ...
            for (int64_t s = 0; s < ns; s++) {
                const int64_t sb0 = s*KC_SB;
                const int64_t nsl = (nsb - sb0) < KC_SB ? (nsb - sb0) : KC_SB;
                kernel3k_16x8(&PA[it*ns + s], &PB[jt*ns + s], nsl, fin);
            }
...
// exact double reference straight from dequantize_row_q3_K semantics
static double dref(int64_t k, const block_q3_K * x, const block_q8_K * y) {
    double acc = 0;
    for (int64_t sb = 0; sb < k/QK_K; sb++) {
        const double d = fp16_to_fp32(x[sb].d);
        const uint32_t kmask1 = 0x03030303, kmask2 = 0x0f0f0f0f;
        uint32_t aux[4]; int8_t sc6[16];
        memcpy(aux, x[sb].scales, 12);
        uint32_t tmp = aux[2];
        aux[2] = ((aux[0] >> 4) & kmask2) | (((tmp >> 4) & kmask1) << 4);
        aux[3] = ((aux[1] >> 4) & kmask2) | (((tmp >> 6) & kmask1) << 4);
        aux[0] = (aux[0] & kmask2) | (((tmp >> 0) & kmask1) << 4);
        ...
        const uint8_t * hm = x[sb].hmask;
        const int8_t * yv = y[sb].qs;
        double bacc = 0;
        int is = 0; uint8_t mbit = 1;
        for (int nn = 0; nn < QK_K; nn += 128) {
            int shift = 0;
            for (int j = 0; j < 4; ++j) {
                for (int half = 0; half < 2; half++) {
                    const double dl = d * (sc6[is++] - 32);
                    for (int l = 0; l < 16; ++l) {
                        const int t = ((q[16*half + l] >> shift) & 3) - ((hm[16*half + l] & mbit) ? 0 : 4);
                        bacc += dl * t * (double)(*yv++);
                    }
                }
                shift += 2; mbit <<= 1;
...
int main() {
    ...
    for (auto & tc : cases) {
        ...
        block_q3_K * A = (block_q3_K*)aligned_alloc(64, m*lda*sizeof(block_q3_K));
        ...
        for (int64_t i = 0; i < m*lda; i++) {
            ...
            for (int b = 0; b < QK_K/8;  b++) A[i].hmask[b] = (uint8_t)(xr() & 0xff);
            for (int b = 0; b < QK_K/4;  b++) A[i].qs[b]    = (uint8_t)(xr() & 0xff);
            for (int b = 0; b < 12;      b++) A[i].scales[b]= (uint8_t)(xr() & 0xff);
        }
        ...
        void * PA = aligned_alloc(64, q3k_apack_size(m, k));
        void * PB = aligned_alloc(64, q3k_bpack_size(n, k));
        q3k_repack_a(A, lda, m, k, PA);
        q3k_pack_b(B, ldb, n, k, PB);
        q3k_gemm_packed(m, n, k, PA, PB, C, ldc, 0, 2);
        q3k_gemm_packed(m, n, k, PA, PB, C, ldc, 1, 2);
        ...
        emax /= scale;
        bool ok = emax < 1e-5;
        printf("q3_K m=%3lld n=%3lld k=%5lld  err=%.3g  %s\n",
